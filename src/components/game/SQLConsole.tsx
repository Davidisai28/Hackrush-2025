...
import { executeQuery, validateQuery, QueryResult } from '../../utils/sqlEngine';
...
export function SQLConsole({ expectedQuery, tableName, onQuerySuccess, onQueryError }: SQLConsoleProps) {
  ...
  const handleExecuteQuery = async () => {
    ...
    if (queryResult.success) {
      const isCorrect = validateQuery(query, expectedQuery);
      if (isCorrect) {
        onQuerySuccess();
      } else {
        onQueryError('La consulta es válida pero no es la respuesta esperada para este reto. Revisa las pistas.');
      }
    } else {
